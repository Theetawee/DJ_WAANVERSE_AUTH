...
class ValidateData:
    ...
    def check_password_confirmation(
        self, password: str, password_confirmation: str
    ) -> Dict[str, Any]:
        """
    ...
    def validate_user_data(
        ...
        Args:
            ...
            password_confirmation (Optional[str], optional): Confirmation password. Defaults to None.
        ...
        email_validation = self.validate_email(email, check_uniqueness=True)
        username_validation = self.validate_username(username, check_uniqueness=True)
        password_validation = self.validate_password(password, username)

        if password_confirmation:
            password_conf_validation = self.check_password_confirmation(
                password, password_confirmation
            )
            if not password_conf_validation["is_valid"]:
                result["errors"]["password_confirmation"] = password_conf_validation[
                    "errors"
                ]
        ...
        if not email_validation["is_valid"]:
            ...
            result["validated_data"]["email"] = email_validation["value"]

        if not username_validation["is_valid"]:
            result["errors"]["username"] = username_validation["errors"]
        else:
            result["validated_data"]["username"] = username_validation["value"]

        if not password_validation["is_valid"]:
            result["errors"]["password"] = password_validation["errors"]
        else:
            result["validated_data"]["password"] = password_validation["value"]
