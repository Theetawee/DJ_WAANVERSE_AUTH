...
class WebAuthnFlowTest(APITestCase):
    def setUp(self):
        ...
        self.verify_passkey_registration_url = reverse(
            "dj_waanverse_auth_verify_registration"
        )
        settings.webauthn_domain = "example.com"
        settings.webauthn_rp_name = "Example"
        settings.webauthn_origin = "https://example.com"
    ...
    def test_verify_passkey_registration_no_challengeId(self):
        self.client.force_authenticate(self.user)
        response = self.client.post(
            self.verify_passkey_registration_url,
        )
