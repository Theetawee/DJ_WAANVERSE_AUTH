...
class VerifyCodeTestCase(APITestCase):
    ...
    def _send_verification_request(self, url, data):
        """Send a verification request (email or phone)."""
        mail.outbox.clear()
    ...
    def test_activate_account_after_signup_phone_number(self):
        """Ensure account activation works after signup."""
        ...
        response = self.client.post(self.active_phone_number, activation_payload)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestResendCode(APITestCase):
    def setUp(self):
        super().setUp()
        mail.outbox.clear()
        self.user = Account.objects.create_user(
            ...
            phone_number="+256779820542",
            phone_number_verified=False,
        )
        self.client.force_authenticate(user=self.user)
        self.send_email_verification_code = reverse(
            "dj_waanverse_auth_send_email_verification_code"
        )
        self.send_phone_number_verification_code = reverse(
            "dj_waanverse_auth_send_phone_number_verification_code"
        )
    ...
    def test_resend_code(self):
        response = self.client.post(
            self.send_email_verification_code, {"email_address": "test1@gmail.com"}
        )
        print(response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(len(mail.outbox), 1)
        response = self.client.post(
            self.send_phone_number_verification_code, {"phone_number": "+256779820542"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
